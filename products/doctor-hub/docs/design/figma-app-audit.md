...
| 26 | (conta) · **Configurações** | `59:2` | ⚠️ (em progresso) | **✅ Tema (claro/escuro/sistema) ENTREGUE** (D-137) — 1º pedaço do gap, puro de UI (sem backend), persiste no dispositivo. **Falta ainda:** Conta&Segurança (alterar senha, sessões) = **auth** (D-109 futura); Notificações (3 toggles vs 2 demo); Preferências **idioma/densidade** (i18n/densidade); Privacidade&LGPD (baixar dados/histórico) = **LGPD/backend**. 🚩 esses pendentes seguem como decisão de produto/segurança. |
