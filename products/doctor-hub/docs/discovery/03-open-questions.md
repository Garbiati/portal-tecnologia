...
- **[Auth] Validar dígito verificador de CPF no LOGIN?** O provider Keycloak resolve o usuário por match de dígitos (não valida DV) — o DV já é validado na CRIAÇÃO (API `Cpf.Valido`). Provavelmente OK (login não precisa), mas registrar caso se queira. (2026-07-05)

- **[Inativação × agendamentos] O que acontece com os AGENDAMENTOS já criados quando um doutor é inativado (global OU num cliente específico)?** _(sondado por agentes na madrugada 2026-07-13; NÃO inferido — pergunta do Alessandro)._ **Hoje: NENHUM tratamento.** Inativar (PATCH `/api/doctors/{id}/status` → `Doctor.Status=Inativo`) ou desabilitar o vínculo (`DoctorVinculo.DisabledAt`) apenas **suspende a oferta FUTURA** (escalas param de gerar vaga — D-186; vaga-por-vínculo some — D-202); os agendamentos existentes ficam intocados e "órfãos de contexto" (`Agendamento` não tem FK p/ `Doctor`, só `MedicoNome` string). **Falta a regra do que FAZER com os futuros:** (a) **manter/honrar** os existentes e só barrar novos — comportamento atual, coerente com **D-192** ("sempre a realidade do cliente" / não-destrutivo); **TRAVEI esse invariante com teste** (`InativacaoAgendamentoTests`) pra ninguém quebrar por acidente; (b) **sinalizar** os futuros como "comprometidos"/em risco → fila de **reagendamento por ausência (D-210, VISÃO)**; (c) **cancelar**; (d) **transferir/realocar** a outro doutor da especialidade (também D-210). **Recomendação:** (a) como default seguro já garantido + (b) quando D-210 for construído. **Sub-lacuna:** a **inativação POR CLIENTE** (o cenário do Alessandro) precisa de um **endpoint** pra setar `DoctorVinculo.DisabledAt` — o modelo e o filtro já existem, falta só o write. Relaciona: D-192, D-210, D-131, D-186.

- **[Indisponibilidade × agendamento existente] Marcar o médico indisponível num dia que já tem agendamento — o que acontece?** _(auditoria lógica 2026-07-13; MESMA classe da inativação acima)._ **Hoje:** `POST /api/indisponibilidades` cria a faixa **sem checar agendamentos existentes** (o próprio código admite "efeitos sobre agendamentos existentes AGUARDAM as regras"). A oferta futura some (dia indisponível não gera vaga — D-186), mas o agendamento já marcado fica intocado. **Falta a regra** (igual à inativação): honrar / sinalizar-p/-reposição (D-151/D-210) / cancelar / bloquear a marcação. **+ BUG de exibição derivável (corrigível):** o mesmo fato fica **inconsistente** — `GET /api/escalas/{id}/dia/{data}` retorna `[]` para dia indisponível (o agendamento **some do modal do dia**, ruim p/ LGPD/operação), MAS os guards D-184 de editar/excluir a escala **continuam contando** esse agendamento (por prefixo de VagaId). "Existe agendamento nesse dia" é verdadeiro (guard) e falso (modal) ao mesmo tempo. Follow-up: fazer o dia-detalhe mostrar o agendamento existente mesmo em dia indisponível (ou alinhar os dois). Relaciona: D-186, D-184, D-210, e a lacuna de inativação acima.
- **[Follow-ups não-bloqueantes da auditoria de agendamento (D-214)]** _(2026-07-13)_ (a) **índice único parcial no banco** em `VagaId` (where Status≠conclusivo) p/ fechar a corrida TOCTOU concorrente-real (hoje a guarda é read-then-write no app — cobre stale/duplo-clique, não 2 requests exatamente simultâneos); (b) `clienteId`-derivado-da-unidade × `tenantEscopo.ClienteId`-do-claim podem **divergir** se um token carregar unidade e clienteId inconsistentes (não explorável pelos papéis atuais — Regulação usa claim, Gestor/Operador usa unidade — mas vale unificar a fonte); (c) alguns testes calculam "próxima segunda a partir de amanhã" e podem cair no mês seguinte nos **últimos dias do mês** → **flaky** por alguns dias/ano; fixar dentro do mês corrente.
