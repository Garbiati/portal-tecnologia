...
- **It.11+ (manutenção, wakes longos):** aguardar o Alessandro. Se houver pedido claro, retomar. Polish de baixo risco só se óbvio. NÃO type ramp / NÃO componentização pesada sem validação.
  ...
  - **It.13 (2026-06-19) — ALESSANDRO RESPONDEU OS 6 BLOQUEADORES → D-093.** Demanda CONFIRMADA: unidade=atendimentos (só Teleconsulta), entrada por health_center(≡cliente) via perfil **Regulação**, periodicidade mensal/semanal, "cobrir"=disponibilizar / falta→**relatório de contratação**, sem paciente (LGPD), digitado. **Figma:** reenquadrei lista+detalhe (health_center/cliente, Regulação, atendimentos, badge "modelo confirmado"); construí **Relatório de contratação** `511:6029` (gap por especialidade + estimativa de médicos flagada) wired do cruzamento. 6 bloqueadores ✅ em `03-open-questions.md`. **75 telas, 100% nav.** Aberto: precedência remanejamento×contratação; atualizar `02-roles.md` com o papel Regulação.
  - **It.14 (manutenção, 2026-06-19):** `02-roles.md` — adicionada pergunta 🔴 sobre **Regulação × Gestor Solicitante** (rename? escopo ampliado? novo?) — NÃO inferi a relação. Figma inalterado (75/75). Aguardando Alessandro nos 2 abertos (precedência remanejamento×contratação; mapeamento do papel) + se quer a tela de **entrada da demanda** pelo Regulação.
