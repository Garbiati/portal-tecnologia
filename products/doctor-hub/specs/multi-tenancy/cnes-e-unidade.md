...
3. **Ingestor CNES + carga inicial** — (a) **fonte pública** (DATASUS **ou** API DEMAS — a decidir), filtro UF∈{PI,AM,AL} + tipos de atendimento, idempotente, job mensal, só-leitura, fonte fake nos testes — **BLOQUEADA por infra GCP** (bucket `cnes-imports` + role; arquitetura D-221); (b) **✅ carga inicial FEITA** (commit `9b5da55`, D-226) — lê `profile_tags`+`profile_tag_groups` da TC (RO só-SELECT, D-069) e materializa Unidades+Grupos+vínculo (D-219), mapa `profile_tag_type`→(Tipo,Natureza) inferido (D-216 §6, liberado p/ homolog); **dedup por CNES** (D-218: 1 cnes = 1 unidade compartilhada + N vínculos); **bootstrap ÚNICO via endpoint admin** `POST /api/admin/carga-teleconsulta/unidades-grupos` (NÃO sync contínuo — alinha D-219, mata a corrida). DEPENDE do **sync de Cliente(HC)** (✅ D-225, commit `acdb1a5`). Ambos **DESLIGADOS** até a credencial RO da TC no homolog. 488 testes; seg APROVADO + eng AJUSTAR→aplicado. 🟡 abertos: sobrescrever curadoria à mão em re-run? Removido pós-bootstrap? reativação.
4. **`Grupo` + vínculos + N:N** (fatiado em 4a/4b/4c por risco):
   ...
   - **4c. a VIRADA** — fatiada em 4c-1 (feito) + lote do operador (adiado) por risco/dependência de Keycloak (D-224):
     - **4c-1. ✅ virada de PLUMBING** (feita, commit `5d56248`) — o **`UnidadeClienteVinculo` vira a ÚNICA fonte de verdade** do cliente; **`Unidade.ClienteId` REMOVIDO** (D-218). Comportamento-preservado (1 vínculo ativo/unidade → derivar == o antigo; **45 testes de isolamento intactos**). `IUnidadeResolver.ResolverClienteIdAsync` com **TRIPWIRE fail-closed** (0 órfã / >1 compartilhada → `null` → query filter barra tudo; nunca arbitra cliente). `TenantEscopo` + `POST /agendamentos` + GET/PUT de unidades derivam do vínculo. Migration dropa `cliente_id` (Down backfill por SQL). 469 testes; seg APROVADO + eng AJUSTAR→aplicado. **Resolve o achado (b) do 7b** (o cliente-alvo do `PUT /grupo` vem do vínculo).
     - **lote do operador (ADIADO** — perto da fatia 3, mexe no Keycloak/identidade): **compartilhamento real 1 CNES→N clientes** (+ merge de duplicados), **escopo do Operador por GRUPO** (§5b) no `POST /agendamentos` + `TenantEscopo`, flag **"ver todos"**. Invariante: operador só vê/agenda nas unidades dos SEUS grupos. **Pré-condição:** o modelo operador↔grupo não existe (só o claim `unidade` hoje) — modelá-lo é o coração deste lote.
...
7. **Gestão de unidades/grupos (endpoints + telas)** — CRUD grupo+unidade (D-219): listar/criar/editar/inativar/excluir-se-sem-vínculo; write-path chama `ProblemaInvariante()`. Gated no vê-tudo (`admin`/`demandas`) por ora; **papel cliente self-service ("Gestor de Contrato") = fatia futura** (mexe no Keycloak).
   ...
   - **7b. ✅ CRUD de `Unidade` (backend)** (feita, commit `2fcb3ce`) — 5 endpoints (listar/criar/editar/mover-de-grupo/inativar); criar seta `Unidade.ClienteId` (1:N transição) + cria `UnidadeClienteVinculo`; invariante I + CNES único entre ativas + código único (case-insensitive, alinha ao resolver do token) + coerência grupo↔cliente; inativar soft-disable o vínculo (contagem coerente). CNES = string validada (base-lookup fica p/ fatia 3). **Sem** hard-delete/desvincular (pós-4c). 464 testes; seg APROVADO. 🟡 **abertos (achados do revisor):** (a) **vincular a grupo INATIVO** deve ser bloqueado? (não inferido — decidir); (b) o `PUT /unidades/{id}/grupo` infere o cliente via `Unidade.ClienteId` — no **4c (N:N)** vai precisar de `clienteId` explícito (contrato de API a desenhar no 7c/4c).
   - **7c. telas** (`doctor-hub-web`) — a gestão é pelas personas **Admin/Demandas (vê-tudo)**, não pelo "Gestor de Contrato" (que é papel futuro/Keycloak). Fatiada:
     - **7c-1. tela de Unidades** — seletor de cliente + lista + criar/editar/inativar/mover-de-grupo. **CNES = campo de texto** (o lookup/busca na base pública é a fatia 3 — D-215; a Unidade não "puxa endereço do CNES" ainda). Rotas `unidades`(Admin)/`demandas-unidades`(Demandas) à la D-178. Passou pelos ritos (ux + coerência + engenharia).
     - **7c-2. ✅ gestão de Grupos** (feita, commit `de3f274` no `doctor-hub-web`) — aba "Grupos" (Tabs Unidades|Grupos abaixo do seletor de cliente compartilhado) com CRUD pela UI: criar/renomear/inativar(soft)/**excluir-se-vazio** (só quando `qtdUnidades===0`; 409 do backend por vínculo histórico → Toast). Fetch de grupos UNIFICADO num `useAsync` robusto (aba consome via `AsyncSection`; seletores derivam ATIVOS com **gate por `loading`** — não exibe grupos do cliente anterior na troca de cliente, evita vínculo cross-tenant); `recarregar` reinvalida grupos+unidades juntos. Passou pelos 3 ritos (eng AJUSTAR→aplicado + ux 3,8/5 + coerência adversarial); 663 testes verdes. **Sem reativar** (questão aberta 7c).
     - **Ainda NÃO nesta fatia (reconcilia a descrição antiga):** "Admin cria Unidade **a partir de um** CNES" (busca na base) = fatia 3; **seletor de 'unidade atual' do operador** = D-217 §5b, depende do escopo-por-grupo (4c).
     - 🟡 abertos (7c): **reativar** grupo/unidade inativado (hoje inativar é caminho só de ida); **busca/filtro** na lista de unidades (adiado — entra quando o volume justificar).
