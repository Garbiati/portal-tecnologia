...
4. **`Grupo` + vínculos + N:N** (⚠️ RISCO ALTO — fronteira de isolamento; revisão humana das invariantes):
   - **4a. ✅ entidade `Grupo`** (feita, commit `944a437`) — tabela `grupos` isolada, FK `Restrict`→`Cliente`, seed demo. SEM referência a Unidade.
   - **4b. vínculo N:N** — `UnidadeClienteVinculo` (D-218) **carregando `GrupoId?`** (o grupo é faceta do vínculo, não da Unidade — a unidade é compartilhada); **índice único parcial** `UNIQUE(CodigoCnes) WHERE Ativo` (reset/reconcilia o seed do modelo 1:N antigo, migra `Unidade.ClienteId`→vínculo); **Operador↔escopo por GRUPO** (§5b) ajustando o escopo do `POST /agendamentos`.
...
7. **Gestão de unidades/grupos (telas + endpoints)** — CRUD grupo+unidade pelo cliente (D-219): listar/criar/editar/inativar/excluir-se-sem-vínculo; write-path chama `ProblemaInvariante()`. **Adicionar índice único parcial `UNIQUE(ClienteId, Nome) WHERE Ativo` no `Grupo`** (evita nome de grupo duplicado por cliente — achado do revisor-engenharia). ⚠️ **Ordem:** só expor CRUD/tela de grupo DEPOIS do 4b (senão `Grupo` fica órfão sem vínculo). Admin cria Unidade a partir de um CNES; seletor de "unidade atual" do operador.
